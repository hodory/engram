...
/**
 * Find all project directories under `~/.claude/projects/` that resolve
 * to the given short project name (including worktree variants).
 *
 * @param {string} projectShortName  Short project name (e.g., "engram")
 * @param {string} [projectsDir]     Override for projects path (testing)
 * @returns {string[]} Matching directory names (not full paths)
 */
export function findProjectDirs(projectShortName, projectsDir = DEFAULT_PROJECTS_DIR) {
  const candidates = listSessionCandidates(projectsDir);
  return candidates.filter(
    (name) => resolveProject(name) === projectShortName,
  );
}

/**
 * Return the default projects directory path.
 *
 * @returns {string} Absolute path to ~/.claude/projects/
 */
