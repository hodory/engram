...
STRIP_PATTERNS = [
    ...
    re.compile(r'<claude-mem-context>.*?</claude-mem-context>', re.DOTALL),
...
def scan_session_metadata(filepath: Path, date_start: datetime, date_end: datetime) -> dict | None:
    """Fast scan: read metadata, count user messages."""
            ...
            for i, line in enumerate(f):
                ...
                if obj.get('type') == 'user' and obj.get('message', {}).get('role') == 'user':
                    ...
                    if first_user_msg is None:
                        ...
                        if cleaned and cleaned.startswith('You are a Claude-Mem'):
                            return None
                ...
                if start_time and i < 5:
                    ...
                    if start_kst >= date_end or start_kst < date_start - timedelta(days=1):
                        return None
    ...
    start_kst = start_time.astimezone(KST)
    if start_kst < date_start or start_kst >= date_end:
        return None
...
def cmd_list(args):
    ...
    for proj_dir in project_dirs:
        ...
        for filepath in jsonl_files:
            try:
                mtime = datetime.fromtimestamp(filepath.stat().st_mtime, tz=KST)
        ...
        for line in f:
            ...
            if ts_str:
                try:
                    ...
                    ts_label = dt.astimezone(KST).strftime('%H:%M')
                except (ValueError, TypeError):
                    pass
...
def main():
    parser = argparse.ArgumentParser(
        description='Recall sessions by date from Claude Code JSONL files (KST)',
    ...
    p_list.add_argument('date_expr', nargs='+', help='Date expression')
    ...
    p_expand.add_argument('--max-msgs', type=int, default=50, help='Max messages to show')
